"""Arista EOS driver (NETCONF). EOS also supports eAPI, but per-intent we use
NETCONF over SSH on port 830 when configured for it.
"""
...
<cli-config-data xmlns="http://arista.com/yang/openconfig/cli"><cmd>{config}</cmd></cli-config-data>
</config>"""
...
class EosDriver(DeviceDriver):
    ...
    async def load_candidate(self, config_text: str) -> CandidateHandle:
        ...
        return CandidateHandle(device=self.device.name, reference="eos-candidate")
