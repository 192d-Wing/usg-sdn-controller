...
<cli-config-data xmlns="http://cisco.com/ns/yang/Cisco-IOS-XE-native"><cmd>{config}</cmd></cli-config-data>
</config>"""


class IosXeDriver(DeviceDriver):
    ...
    async def load_candidate(self, config_text: str) -> CandidateHandle:
        ...
        return CandidateHandle(device=self.device.name, reference="iosxe-candidate")
